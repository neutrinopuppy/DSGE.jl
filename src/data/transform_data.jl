...
"""
```
function transform_spd_data(spd_df::DataFrame)
```
...
function transform_spd_data(spd_df::DataFrame)
    ...
    new_df = combine(gd, :MODAL_MEDIAN => mean, renamecols = false)
    ...
    insertcols!(new_df, :final_vals => new_df[!,:MODAL_MEDIAN])

    sort!(new_df, [:date, :qtrs_ahead])
    insertcols!(new_df, :prev_val => vcat(new_df[1,:MODAL_MEDIAN], new_df[1:end-1,:MODAL_MEDIAN]))
    ...
    #last_meeting = Vector{Date}(undef, length(find_date_qtr))
    ...
    ## Aggregate using implied daily FFR
    ...
    for i in 1:length(gd2)
        if nrow(gd2[i]) > 1 && all(occursin.("/", gd2[i][!,:PREDICTION_HORIZON_DATE]))
            ...
            gd2[i][!,:final_vals] .= (sum(fomc_diffs .* gd2[i][1:end-1,:MODAL_MEDIAN]) +
                                      (gd2[i][end,:predicted_qtr] - horizon_dates[end]) / Dates.Day(1) * gd2[i][end,:MODAL_MEDIAN] +
        ...
        elseif nrow(gd2[i]) > 1 && any(occursin.("/", gd2[i][!,:PREDICTION_HORIZON_DATE]))
            if any(occursin.("/", gd2[i][!,:PREDICTION_HORIZON_DATE]))
                ...
                subgd = combine(groupby(gd2[i], :PREDICTION_HORIZON_DATE), :MODAL_MEDIAN => mean,
                ...
                final_val = (sum(fomc_diffs .* subgd[1:end-1,:MODAL_MEDIAN_mean]) +
                             (subgd[end,:predicted_qtr_function] - horizon_dates[end]) / Dates.Day(1) * subgd[end,:MODAL_MEDIAN_mean] +
    ...
    names(new_df2)[2:end] = "exp_ant" .* names(new_df2)[2:end]
