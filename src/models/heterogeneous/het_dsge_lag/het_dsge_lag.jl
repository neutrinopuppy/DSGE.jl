...
"""
mutable struct HetDSGELag{T} <: AbstractModel{T}
...
description(m::HetDSGELag) = "HetDSGELag, $(m.subspec)"

"""
`init_model_indices!(m::HetDSGELag)`

Arguments:
`m:: HetDSGELag`: a model object

Description:
Initializes indices for all of `m`'s states, shocks, and equilibrium conditions.
"""
function init_model_indices!(m::HetDSGELag)
    # Endogenous states
...
those).
"""
function init_parameters!(m::HetDSGELag)
...
"""
```
aggregate_steadystate!(m::HetDSGELag)
```
...
"""
function aggregate_steadystate!(m::HetDSGELag)
...
"""
```
init_grids!(m::HetDSGELag)
```
"""
function init_grids!(m::HetDSGELag)
...
function model_settings!(m::HetDSGELag)
    default_settings!(m)
...
function setup_indices!(m::HetDSGELag)
    nx = get_setting(m, :nx)
    ns = get_setting(m, :ns)
    endo = m.endogenous_states_unnormalized
 ...
 # function-valued jumps
    ...
    endo[:mc′_t] = 4*nxns+33:4*nxns+33        # marginal cost - this is ζ in HetDSGELagₖd.pdf
