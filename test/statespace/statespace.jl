...
@testset "Calculating transition matrices/vectors for k-periods ahead expectations and expected sums" begin
    ...
    for i in 2:6
        regime_eqcond_info[i] = DSGE.EqcondEntry(DSGE.zlb_rule(), [1., 0.])
    end
...
@testset "Time-Varying Information Set state space system" begin
    ...
    for i in 2:6
        regime_eqcond_info1[i] = DSGE.EqcondEntry(DSGE.zlb_rule())#, [1., 0.])
    end
...
@testset "Using gensys2 in historical regimes" begin
    ...
    custom_set = [Setting(:n_mon_anticipated_shocks, 6, "Number of anticipated policy shocks"),
                  Setting(:imperfect_awareness_weight, [imperfect_cred_new, imperfect_cred_old]),
...
@testset "Measurement equation of forward-looking variables" begin
    ...
    imperfect_cred_old = 1. - imperfect_cred_new
    custom_set = [Setting(:n_mon_anticipated_shocks, 6, "Number of anticipated policy shocks"),
                  Setting(:imperfect_awareness_weights, [imperfect_cred_new, imperfect_cred_old]),
                  Setting(:alternative_policies, [DSGE.taylor_rule()]),
                  Setting(:flexible_ait_policy_change, true)]
    ...
    for (regind, date) in zip(4:(4 + 4), # 4 + 1 b/c zero for 4 periods and liftoff, add 3 to make correct regime
        ...
        reg_dates[regind] = date
        if regind != 4 + 4
            regime_eqcond_info[regind] = DSGE.EqcondEntry(DSGE.zlb_rule(), [imperfect_cred_new, imperfect_cred_old])
        end
    end
    regime_eqcond_info[8] = DSGE.EqcondEntry(DSGE.flexible_ait(), [imperfect_cred_new, imperfect_cred_old])
